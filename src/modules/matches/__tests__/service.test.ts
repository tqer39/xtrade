...
describe('matches/service', () => {
  ...
  describe('findMatches', () => {
    ...
    it('limit と offset が適用される', async () => {
      ...
      mockWhere.mockResolvedValueOnce([{ userId: 'user-2' }, { userId: 'user-3' }]);
      // 自分のカードを欲しがっているユーザー
      mockWhere.mockResolvedValueOnce([]);
      // 候補ユーザーの詳細
      mockWhere.mockResolvedValueOnce([
    ...
    it('マッチスコアでソートされる', async () => {
      // 持っているカード
      mockWhere.mockResolvedValueOnce([{ cardId: 'card-1' }, { cardId: 'card-2' }]);
      // 欲しいカード
      mockWhere.mockResolvedValueOnce([{ cardId: 'card-3' }, { cardId: 'card-4' }]);
      // 欲しいカードを持っているユーザー
      mockWhere.mockResolvedValueOnce([{ userId: 'user-2' }, { userId: 'user-3' }]);
      // 自分のカードを欲しがっているユーザー
      mockWhere.mockResolvedValueOnce([]);
