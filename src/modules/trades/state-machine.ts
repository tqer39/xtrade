 ...
 * - 'both': どちらでも可
 */
const TRANSITION_PERMISSIONS: Record<string, 'initiator' | 'responder' | 'both'> = {
 ...
 */
export function validateTransition(trade: Trade, toStatus: TradeStatus, userId: string): void {
  // 1. 遷移の妥当性チェック
  ...
  if (requiredPermission) {
    ...
    if (requiredPermission === 'initiator' && !isInitiator) {
      throw new TradeTransitionError('Only the initiator can perform this action', 'UNAUTHORIZED');
    }

    if (requiredPermission === 'responder' && !isResponder) {
      throw new TradeTransitionError('Only the responder can perform this action', 'UNAUTHORIZED');
    }
  }
  ...
  if (trade.status === 'proposed' && trade.proposedExpiredAt) {
    if (new Date() > trade.proposedExpiredAt) {
      throw new TradeTransitionError('This trade has expired', 'EXPIRED');
    }
  }
