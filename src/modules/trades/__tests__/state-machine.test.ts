...
import { canParticipate, canTransition, isFinalStatus, validateTransition } from '../state-machine';
...
describe('canTransition', () => {
  ...
  describe('終了状態からの遷移', () => {
    const finalStatuses: TradeStatus[] = ['completed', 'disputed', 'canceled', 'expired'];
...
describe('validateTransition', () => {
  describe('権限チェック', () => {
    it('開始者が draft → proposed を実行できる', () => {
      ...
      expect(() => validateTransition(trade, 'proposed', 'user-initiator')).not.toThrow();
    });

    it('応答者は draft → proposed を実行できない', () => {
      const trade = createTrade({ status: 'draft' });
      expect(() => validateTransition(trade, 'proposed', 'user-responder')).toThrow(
    ...
    it('応答者が proposed → agreed を実行できる', () => {
      ...
      expect(() => validateTransition(trade, 'agreed', 'user-responder')).not.toThrow();
    });

    it('開始者は proposed → agreed を実行できない', () => {
      const trade = createTrade({ status: 'proposed' });
      expect(() => validateTransition(trade, 'agreed', 'user-initiator')).toThrow(
        TradeTransitionError
      );
    });
    ...
    it('両者とも proposed → canceled を実行できる', () => {
      const trade = createTrade({ status: 'proposed' });
      expect(() => validateTransition(trade, 'canceled', 'user-initiator')).not.toThrow();
      expect(() => validateTransition(trade, 'canceled', 'user-responder')).not.toThrow();
    });

    it('両者とも agreed → completed を実行できる', () => {
      const trade = createTrade({ status: 'agreed' });
      expect(() => validateTransition(trade, 'completed', 'user-initiator')).not.toThrow();
      expect(() => validateTransition(trade, 'completed', 'user-responder')).not.toThrow();
    });

    it('両者とも agreed → disputed を実行できる', () => {
      const trade = createTrade({ status: 'agreed' });
      expect(() => validateTransition(trade, 'disputed', 'user-initiator')).not.toThrow();
      expect(() => validateTransition(trade, 'disputed', 'user-responder')).not.toThrow();
    });

    it('参加者以外は遷移を実行できない', () => {
      const trade = createTrade({ status: 'proposed' });
      expect(() => validateTransition(trade, 'canceled', 'user-other')).toThrow(
        TradeTransitionError
      );
    });
  });
  ...
  describe('無効な遷移', () => {
    ...
    it('completed からの遷移はエラー', () => {
      ...
      expect(() => validateTransition(trade, 'draft', 'user-initiator')).toThrow(
        TradeTransitionError
      );
    });
  });
  ...
  describe('期限チェック', () => {
    it('proposed 状態で期限切れの場合はエラー', () => {
      ...
      expect(() => validateTransition(trade, 'agreed', 'user-responder')).toThrow(
        TradeTransitionError
      );
    });
    ...
    it('agreed 状態で期限切れの場合はエラー', () => {
      ...
      expect(() => validateTransition(trade, 'completed', 'user-initiator')).toThrow(
        TradeTransitionError
      );
    });
