...
describe('trades/service', () => {
  ...
  describe('updateOffer', () => {
    it('参加者以外はエラー', async () => {
      ...
      await expect(
        updateOffer(trade, 'user-3', { items: [] })
      ).rejects.toThrow(TradeTransitionError)
    })
    ...
    it('draft 状態でオファー更新可能', async () => {
      ...
      await expect(
        updateOffer(trade, 'user-1', { items: [] })
  ...
  describe('setResponder', () => {
    ...
    it('既に応答者がいる場合はエラー', async () => {
      ...
      await expect(setResponder(trade, 'user-3')).rejects.toThrow(
    ...
    it('開始者と同じユーザーを応答者にはできない', async () => {
      ...
      await expect(setResponder(trade, 'user-1')).rejects.toThrow(
        TradeTransitionError
      )
    })
  })
