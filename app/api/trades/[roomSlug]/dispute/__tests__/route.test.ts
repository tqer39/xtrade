...
function createMockRequest(body?: Record<string, unknown>): NextRequest {
  ...
  return {
    method: 'POST',
    url: 'http://localhost:3000/api/trades/abc123/dispute',
    text: () => Promise.resolve(bodyText),
  } as unknown as NextRequest;
}

describe('/api/trades/[roomSlug]/dispute', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  });
  ...
  describe('POST - 紛争報告', () => {
    ...
    it('紛争を報告できる', async () => {
      ...
      expect(mockTransitionTrade).toHaveBeenCalledWith(mockTrade, 'disputed', 'user-1', {
        reason: undefined,
      });
    });

    it('理由を指定して紛争を報告できる', async () => {
      const mockTrade = { id: 'trade-1', roomSlug: 'abc123', status: 'agreed' };
      mockGetSession.mockResolvedValue({
        user: { id: 'user-1' },
      });
      ...
      const { POST } = await import('../route');
      const request = createMockRequest({ reason: 'カードの状態が説明と異なる' });
      const response = await POST(request, { params: Promise.resolve({ roomSlug: 'abc123' }) });
      const json = await response.json();
