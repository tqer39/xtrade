...
function createMockRequest(body?: Record<string, unknown>): NextRequest {
  ...
  return {
    method: 'POST',
    url: 'http://localhost:3000/api/trades/abc123/cancel',
    text: () => Promise.resolve(bodyText),
  } as unknown as NextRequest;
}

describe('/api/trades/[roomSlug]/cancel', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  });

  describe('POST - トレードキャンセル', () => {
    ...
    it('トレードをキャンセルできる', async () => {
      ...
      expect(mockTransitionTrade).toHaveBeenCalledWith(mockTrade, 'canceled', 'user-1', {
        reason: undefined,
      });
    });
    ...
    it('理由を指定してキャンセルできる', async () => {
      ...
      const { POST } = await import('../route');
      const request = createMockRequest({ reason: 'カードが見つからない' });
      const response = await POST(request, { params: Promise.resolve({ roomSlug: 'abc123' }) });
      const json = await response.json();
