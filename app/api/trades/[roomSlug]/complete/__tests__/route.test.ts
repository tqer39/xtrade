...
function createMockRequest(): NextRequest {
  return {
    method: 'POST',
    url: 'http://localhost:3000/api/trades/abc123/complete',
  } as unknown as NextRequest;
}

describe('/api/trades/[roomSlug]/complete', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    vi.resetModules();
  });

  describe('POST - トレード完了', () => {
    ...
    it('権限がない場合は 403 を返す', async () => {
      const mockTrade = { id: 'trade-1', roomSlug: 'abc123', status: 'agreed' };
      mockGetSession.mockResolvedValue({
        user: { id: 'user-3' },
      });
    ...
    it('期限切れの場合は 410 を返す', async () => {
      ...
      mockGetTradeByRoomSlug.mockResolvedValue(mockTrade);
      mockTransitionTrade.mockRejectedValue(
        new MockTradeTransitionError('Trade expired', 'EXPIRED')
      );
      ...
      expect(json.error).toBe('Trade expired');
    });
    ...
    it('トレードを完了できる', async () => {
      ...
      expect(mockTransitionTrade).toHaveBeenCalledWith(mockTrade, 'completed', 'user-1');
    });
  });
});
